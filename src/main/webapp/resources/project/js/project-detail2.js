...
$(function(){
	...
	var url = $("#basePath").val()+"/project/subject/list";
	doGetAjax(url, data, doGetDetailBack);
	queryTableData();
	
	//返回
	$('#backBtn').click(function() {
		location.href = $("#basePath").val()+"/project/flow_project.htm";
	});
...
function doGetDetailBack(res){
	if (res.success == true) {
		if(res.data != null){
			...
			$("#serve").html(serveFormatter(result.serve));
			$("#quote").html(quoteFormatter(result.quote));
			...
			$("#trader").html(traderFormatter(result.trader));
		   	$("#unstartDatetime").html(dateFormatter(result.unstartDatetime));
