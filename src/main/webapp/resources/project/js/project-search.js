...
$(function(){
	...
	$("#status").renderDropdown(Dict.getName("subject_status"));
    //页面数据字典初始化
//	initData();
	//分页查询
	queryTableData();
	...
	$('#detailBtn').click(function() {
		...
		window.location.href = $("#basePath").val()+"/project/project_detail.htm?subjectCode="+selRecords[0].code+"&op=search";
	});
	// 编辑菜单绑定事件
	$('#finishBtn').click(function() {
		...
		window.location.href = $("#basePath").val()+"/project/finish_project_detail.htm?subjectCode="+selRecords[0].code;
	});
	// 查询事件绑定
